...
print(minitest("HOPE", data_random_HOPELESSA
         ))


suimed_outcomes <- c("AGITATION",
                   "ANGER",
                   "ANXIETY",
                   "PERCEIVEDBURDEN",
                   "DEPRESSION",
...
testsuimedoutcomes <- c("DEPRESSION") %>% noquote()
...
#checking why it seems cut off at the bottom
data_random_cleaned_SI %>% filter(perimens_v_periov_plusfixed <= -2) %>% select(perimens_v_periov_plusfixed)
...
#run plot code on all items



====archive====
### below codeis to do one rain cloud plot by hand

#step 1. data
```{r}
data_randomeffects_DepA <- read_csv("JACKIE SUIC MED raincloud plots/AIM 2 RAINCLOUD PLOTS/Aim 2 Random Effects Files for Plots (For Jordan)/DEPA.csv")
data_randomeffects_DepB <- read_csv("JACKIE SUIC MED raincloud plots/AIM 2 RAINCLOUD PLOTS/Aim 2 Random Effects Files for Plots (For Jordan)/DEPB.csv")
data_randomeffects_DepC <- read_csv("JACKIE SUIC MED raincloud plots/AIM 2 RAINCLOUD PLOTS/Aim 2 Random Effects Files for Plots (For Jordan)/DEPC.csv")
data_randomeffects_DepD <- read_csv("JACKIE SUIC MED raincloud plots/AIM 2 RAINCLOUD PLOTS/Aim 2 Random Effects Files for Plots (For Jordan)/DEPD.csv")
```
...
##ref group A: perimenstrual
#remove duplicates/extraneous
data_randomeffects_DepA <- data_randomeffects_DepA %>%
  select(-c(DEPRESSION, starts_with("LH_"),
            ends_with("_SE"))) %>%
  relocate(ID, DEP_RANDOM_Midfol, DEP_RANDOM_Periov, DEP_RANDOM_EarlyLut, DEP_RANDOM_MidLut) %>%
  #rename the predictor variables to denote what their reference group is
  rename(perimens_v_midfol = DEP_RANDOM_Midfol,
         perimens_v_periov = DEP_RANDOM_Periov,
         perimens_v_earlylut = DEP_RANDOM_EarlyLut,
         perimens_v_midlut = DEP_RANDOM_MidLut) %>%
   #just want 1 fixed effect per person (these were copied down for each daily rating)
  group_by(ID) %>% slice(n=1) %>%
  #MANUALLY add the fixed effect from the model to each random effect as a new variable
  mutate(perimens_v_midfol_plusfixed = perimens_v_midfol-0.274,
         perimens_v_periov_plusfixed = perimens_v_periov-0.245,
         perimens_v_earlylut_plusfixed = perimens_v_earlylut-0.295,
         perimens_v_midlut_plusfixed = perimens_v_midlut-0.087)

##ref group B: midluteal
#remove duplicates/extraneous
#--> note here that the duplicate contrast we are removing is because already got "perimens vs midluteal" with perimens as reference group, so we don't need "midlut vs perimens"
data_randomeffects_DepB <- data_randomeffects_DepB %>%
  select(-c(DEPRESSION, starts_with("LH_"), 
            ends_with("_SE"),
            DEP_RANDOM_perimen, DEP_RANDOM_perimen_SE)) %>%
  relocate(ID, DEP_RANDOM_Midfol, DEP_RANDOM_Periov, DEP_RANDOM_EarlyLut) %>%
  #rename the predictor variables to denote what their reference group is
  rename(midlut_v_midfol = DEP_RANDOM_Midfol,
         midlut_v_periov = DEP_RANDOM_Periov,
         midlut_v_earlylut = DEP_RANDOM_EarlyLut) %>%
   #just want 1 fixed effect per person (these were copied down for each daily rating)
  group_by(ID) %>% slice(n=1) %>%
  #MANUALLY add the fixed effect from the model to each random effect as a new variable
  mutate(midlut_v_midfol_plusfixed = midlut_v_midfol-0.187,
         midlut_v_periov_plusfixed = midlut_v_periov-0.158,
         midlut_v_earlylut_plusfixed = midlut_v_earlylut-0.208)

##ref group C: early luteal
#remove duplicates/extraneous
data_randomeffects_DepC <- data_randomeffects_DepC %>%
  select(-c(DEPRESSION, starts_with("LH_"), 
            ends_with("_SE"),
            DEP_RANDOM_perimen, DEP_RANDOM_perimen_SE,
            DEP_RANDOM_MidLut, DEP_RANDOM_Midlut_SE)) %>%
  relocate(ID, DEP_RANDOM_Midfol, DEP_RANDOM_Periov) %>%
  #rename the predictor variables to denote what their reference group is
  rename(earlylut_v_midfol = DEP_RANDOM_Midfol,
         earlylut_v_periov = DEP_RANDOM_Periov) %>%
   #just want 1 fixed effect per person (these were copied down for each daily rating)
  group_by(ID) %>% slice(n=1) %>%
  #MANUALLY add the fixed effect from the model to each random effect as a new variable
  mutate(earlylut_v_midfol_plusfixed = earlylut_v_midfol+0.021,
         earlylut_v_periov_plusfixed = earlylut_v_periov-0.05)

##ref group D: periov
#remove duplicates/extraneous
data_randomeffects_DepD <- data_randomeffects_DepD %>%
  select(-c(DEPRESSION, starts_with("LH_"), 
            ends_with("_SE"),
            DEP_RANDOM_perimen, DEP_RANDOM_perimen_SE,
            DEP_RANDOM_MidLut, DEP_RANDOM_Midlut_SE,
            DEP_RANDOM_EarlyLut, DEP_RANDOM_EarlyLut_SE)) %>%
  relocate(ID, DEP_RANDOM_Midfol) %>%
  #rename the predictor variables to denote what their reference group is
  rename(periov_v_midfol = DEP_RANDOM_Midfol) %>%
   #just want 1 fixed effect per person (these were copied down for each daily rating)
  group_by(ID) %>% slice(n=1) %>%
  #MANUALLY add the fixed effect from the model to each random effect as a new variable
  mutate(periov_v_midfol_plusfixed = periov_v_midfol-0.029)
...
suimed_data_random_AB <- full_join(data_randomeffects_DepA, data_randomeffects_DepB, by="ID")
suimed_data_random_ABC <- full_join(suimed_data_random_AB, data_randomeffects_DepC, by="ID")
suimed_data_random_ABCD <- full_join(suimed_data_random_ABC, data_randomeffects_DepD, by="ID")
...
suimed_raincloud_DEP <- suimed_data_random_ABCD %>%
  #pivoting is necessary to get ALL random effects as one variable, rather than 6 columns
  pivot_longer(
    cols = ends_with("plusfixed"),
    names_to = "pairwise",
  ...
  theme(axis.text.x = element_blank(),
        axis.title.x = element_blank())+
  ylab("Fixed + Random Effect")+
  ggtitle("individual differences of cycle phase on depression")+
  labs(fill="Cycle Phase Contrast")+
  geom_hline(yintercept = 0, linetype="dashed", color="gray")+
  geom_text(aes(x=1, y=-2), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="  (-)higher DEP \n in E.LUT")+
  geom_text(aes(x=1, y=3), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="    (+)higher DEP \n in MIDFOL")+
  geom_text(aes(x=2, y=-2.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in E.LUT")+
  geom_text(aes(x=2, y=3.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in PERIOV")+
  geom_text(aes(x=3, y=-2), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in MIDLUT")+
  geom_text(aes(x=3, y=3), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in E.LUT")+
  geom_text(aes(x=4, y=-2.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in MIDLUT")+
  geom_text(aes(x=4, y=3.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in MIDFOL")+
  geom_text(aes(x=5, y=-2), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in MIDLUT")+
  geom_text(aes(x=5, y=3), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in PERIOV")+
  geom_text(aes(x=6, y=-2.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in PERIMENS")+
  geom_text(aes(x=6, y=3.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in E.LUT")+
  geom_text(aes(x=7, y=-2), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in PERIMENS")+
  geom_text(aes(x=7, y=3), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in MIDFOL")+
  geom_text(aes(x=8, y=-2.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in PERIMENS")+
  geom_text(aes(x=8, y=3.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in MIDLUT")+
  geom_text(aes(x=9, y=-2), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in PERIMENS")+
  geom_text(aes(x=9, y=3), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in PERIOV")+
  geom_text(aes(x=10, y=-2.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(-)higher DEP \n in PERIOV")+
  geom_text(aes(x=10, y=3.25), check_overlap = TRUE, fontface="italic", size=3,color="black",
            label="(+)higher DEP \n in MIDFOL")
suimed_raincloud_DEP %>% ggsave(filename = "2022-08-02 suimed_ranef_dep.png", 
                                                width = 12, height = 5,
                                                units = c("in"),
                                                device = "png")
```

# plot variables by attempter status

## import trait data, merge, clean, create summary variable
**If lifetime attempt, lifetime aborted attempt, or lifetime interrupted attempt equal 1 --> "LT_attempt_any_yn" ==1**
```{r}

suimednonmissing <- read_sav("Mergeddaily_Final.sav")
c1trait <- read_sav("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CLEAR1/02_datasets/CLEAR1_TRAIT/03_cleaned_data/2022-01-06_CLEAR1_TRAIT.sav")
c2trait <- read_sav("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CLEAR2/02_datasets/CLEAR2_TRAIT/03_cleaned_data/2021-12-29_clear2traitscid.sav")
c3trait <- read_sav("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CLEAR3/02_datasets/CLEAR3_TRAIT/03_cleaned_data/2022-08-16 clear3traitscidsitbi.sav")

#little formatting things that need to be fixed to merge trait (pulled from script cycle-etoh-03)
c1trait$rel_years <-as.character(c1trait$rel_years)
c2trait$diet <-as.character(c2trait$diet)
c2trait$numpregnancies <-as.numeric(c2trait$numpregnancies)
c1trait$PPDpregmnths <- as.numeric(c1trait$PPDpregmnths)
c2trait$SES_occupation <- as.character(c2trait$SES_occupation)
c2trait <- c2trait %>% mutate(across(starts_with("SES_zipcode"), as.character))
c2trait$age <- as.numeric(c2trait$age)
c1trait$age <- as.numeric(c1trait$age)
c3trait <- c3trait %>% rename(age=Age,
                              LTattemptyn = Attempt_yn,
                              LTabattyn = AbortedAtt_yn,
                              LTintattyn = Interrupted_Att_yn)

#remove the trait scales that we know are not needed here
c1trait <- c1trait %>% select(c(1:133,
                     starts_with("current"),
                     starts_with("lifetime"),
                     starts_with("LT")))
c2trait <- c2trait %>% select(c(1:133,
                     starts_with("current"),
                     starts_with("lifetime"),
                     starts_with("LT")))
...
#merge trait
temptrait <- full_join(c1trait, c2trait)
trait <- full_join(temptrait, c3trait)
trait <- trait %>% arrange(id)

trait$LTattemptyn
trait$LTabattyn
trait$LTintattyn

trait %>% mutate(LTattempt_any_yn = ifelse(LTattemptyn==1 |
                                             LTintattyn==1 | 
                                             LTattemptyn ==1, 1, 0)) %>% 
  select(id, LTattemptyn, LTabattyn, LTintattyn, LTattempt_any_yn)

trait <- trait %>% mutate(LTattempt_any_yn = ifelse(LTattemptyn==1 |
                                             LTintattyn==1 | 
                                             LTattemptyn ==1, 1, 0))

```

#plot outcomes by LT attempt status
```{r}
suimeddailytrait <- left_join(suimeddata_cycleday, trait)


testplot_LTattempt1 <- suimeddailytrait %>% filter(LTattemptyn==1) %>%
  group_by(hybridcycleday) %>%
  summarize(personcentered.dev = mean(SI_Final.d, na.rm=T),
            se=sd(SI_Final.d, na.rm=T)/sqrt(n())) %>%
  arrange(factor(hybridcycleday, levels = hybridcycleday_order)) %>%
  mutate(roll= rollapply(personcentered.dev, 3, 
                         mean, align = "center", fill=NA)) %>%
    mutate(LTattemptyn= "1")

testplot_LTattempt0 <- suimeddailytrait %>% filter(LTattemptyn==0) %>%
  group_by(hybridcycleday) %>%
  summarize(personcentered.dev = mean(SI_Final.d, na.rm=T),
            se=sd(SI_Final.d, na.rm=T)/sqrt(n())) %>%
  arrange(factor(hybridcycleday, levels = hybridcycleday_order)) %>%
  mutate(roll= rollapply(personcentered.dev, 3, 
                         mean, align = "center", fill=NA)) %>%
    mutate(LTattemptyn= "0")

data_suimed_by_LTattempt <- full_join(testplot_LTattempt0, testplot_LTattempt1)


# New facet label names for faceting variable
facet.labs <- c("No lifetime suicide attempt", "Lifetime suicide attempt* (*aborted, interrupted, or actual)")
names(facet.labs) <- c("0", "1")


data_suimed_by_LTattempt %>%
  ggplot(aes(x=factor(hybridcycleday, levels = hybridcycleday_order), y=roll,
             group=LTattemptyn,fill=LTattemptyn))+
  geom_line()+
  geom_ribbon(aes(ymin=roll-se,
                  ymax=roll+se),
                  alpha=0.2)+
  facet_wrap(~LTattemptyn, labeller = labeller(LTattemptyn=facet.labs))+
  ylab("Person-centered SI (3-day rolling average)")+
  xlab("Cycle Day (hybrid count)")+
  theme_bw()+
  guides(fill=FALSE)+
...
===archive
### plot just asiq9 as an example of line graph by cycle phase then cycle day
```{r}


suimeddata <- suimeddata %>%
  group_by(id) %>%
  mutate(ASIQ9_wishdead.m = mean(ASIQ9_wishdead, na.rm = TRUE))
suimed_data_for_plot1a_asiq9 <- suimeddata %>% 
  group_by(id, LH_cyclephasenames) %>% 
  summarize(ASIQ9_wishdead.m=mean(ASIQ9_wishdead.m),
            ASIQ9_wishdead.phase.m = mean(ASIQ9_wishdead, na.rm = TRUE),
            ASIQ9_wishdead.phase.d = ASIQ9_wishdead.phase.m - ASIQ9_wishdead.m)


suimed_data_for_plot1a_asiq9 %>% 
  group_by(LH_cyclephasenames) %>%
  summarize(grand.m.ASIQ9_wishdead.d = mean(ASIQ9_wishdead.phase.d, na.rm=T),
            se=sd(ASIQ9_wishdead.phase.d, na.rm=T)/sqrt(n())) %>%
  ggplot(aes(x=LH_cyclephasenames, y=grand.m.ASIQ9_wishdead.d))+
  geom_line(group=1)+
  scale_x_discrete(labels=c("midfol" = "Midfollicular", 
                            "midlut" = "Midluteal",
                            "earlylut" = "Early Luteal",
                            "perimen" = "Perimenstrual",
                            "periov" = "Periovulatory"))+
  geom_errorbar(aes(ymin=grand.m.ASIQ9_wishdead.d-se, 
                    ymax=grand.m.ASIQ9_wishdead.d+se), 
                size=0.5, width=0.5, position=position_dodge())+
  geom_hline(yintercept=0.0, color="red", linetype="dashed")+
  theme_bw(base_size = 11)+
  xlab("Cycle Phase")+
  ylab("Person-Centered ASIQ9")

#run cycle day plot on asiq9
suimed_data_for_plot_asiq9 <- suimeddata_cycleday %>% group_by(id, hybridcycleday) %>% 
  filter(!is.na(hybridcycleday)) %>%
  summarize(ASIQ9_wishdead.m=mean(ASIQ9_wishdead.m),
            ASIQ9_wishdead.day.m = mean(ASIQ9_wishdead, na.rm = TRUE),
            ASIQ9_wishdead.day.d = ASIQ9_wishdead.day.m - ASIQ9_wishdead.m) %>%
  arrange(factor(hybridcycleday, levels = hybridcycleday_order))

suimed_data_for_plot_asiq9 %>% group_by(hybridcycleday) %>%
  summarize(personcentered.dev = mean(ASIQ9_wishdead.day.d, na.rm=T),
            se=sd(ASIQ9_wishdead.day.d, na.rm=T)/sqrt(n())) %>%
  arrange(factor(hybridcycleday, levels = hybridcycleday_order)) %>%
  mutate(roll= rollapply(personcentered.dev, 3, 
                         mean, align = "center", fill=NA)) %>%
  ggplot(aes(x=factor(hybridcycleday, levels = hybridcycleday_order), y=roll))+
