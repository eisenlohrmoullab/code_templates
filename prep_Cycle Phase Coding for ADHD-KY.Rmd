...
#check overlaps: goal is to only have 5 possible group combinations here (1 row per dummy coded phase, then a row where ALL are NA)
Clear2DailyData %>%
  group_by(periov_LH, midluteal_LH, perimenstrual_LH, midfol_LH) %>%
  summarize(n=n())
#make note of errors
...
Clear2DailyData %>%
  ...
  select(id, daterated, posLHtoday, daycountLH,
         ...
         LH_midfol_LH, LH_periov_LH,
         LH_midluteal_count, LH_perimenstrual_count, cleartrialphase) %>%
  View()
